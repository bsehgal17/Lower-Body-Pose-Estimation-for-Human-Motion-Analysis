...
def main():
    ...
    scores_parser = subparsers.add_parser(
        "scores", help="Extract for specific PCK scores"
    )
    ...
    compare_parser = subparsers.add_parser(
        "compare", help="Compare brightness across scores"
    )
    compare_parser.add_argument(
        "score_list", nargs="+", type=int, help="List of PCK scores to compare"
    )
    ...
    try:
        extractor = BrightnessExtractor(args.dataset)

        if args.command == "scores":
            brightness_data = extractor.extract_brightness_for_scores(
            ...
            print("\n📊 Detailed Brightness Statistics:")
            for score, stat_data in stats.items():
                print(f"\nPCK Score {score}:")
                for stat_name, value in stat_data.items():
