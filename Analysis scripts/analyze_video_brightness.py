...
def analyze_video(video_path, output_dir):
    ...
    # Add regional stats if enabled
    if REGIONAL_ANALYSIS:
        for region_pos in ['1-1', '1-2', '1-3', '2-1', '2-2', '2-3', '3-1', '3-2', '3-3']:
            region_means = [f['regions'][int(region_pos[0])-1][int(region_pos[2])-1]['mean']
                            for f in frame_data if 'regions' in f]
...
def analyze_video_folder(folder_path):
    ...
    if all_results:
        ...
        with open(os.path.join(output_dir, "summary_report.txt"), 'w') as f:
            ...
            f.write("Overall brightness statistics:\n")
