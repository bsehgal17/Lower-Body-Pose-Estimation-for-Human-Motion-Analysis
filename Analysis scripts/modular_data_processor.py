...
class ModularDataProcessor(BaseDataProcessor):
    ...
    def load_pck_scores(self) -> pd.DataFrame:
        ...
        return self._load_excel_sheet('Overall Metrics', self.config.pck_overall_score_columns)
    ...
    def _load_excel_sheet(self, sheet_name: str, required_score_columns: List[str]) -> pd.DataFrame:
        """Generic method to load Excel sheets with validation."""
        try:
            df = pd.read_excel(self.config.pck_file_path, sheet_name=sheet_name, header=0)
            ...
            if self.config.camera_column and self.config.camera_column in df.columns:
                df[self.config.camera_column] = df[self.config.camera_column].astype(int)
    ...
    def process_overall_data(self, pck_df: pd.DataFrame, metrics_config: Dict[str, str]) -> Dict[str, Any]:
        ...
        for metric_name, extractor_method in metrics_config.items():
            ...
            if not grouping_cols:
                print("Warning: No grouping columns found. Cannot perform per-video analysis.")
                continue
            ...
            for group_name, group_data in grouped_pck_df:
                ...
                extractor = MetricExtractorFactory.create_extractor(metric_name, video_path)
            ...
            if video_metrics_rows:
                ...
                merged_df = pd.merge(pck_df, video_metrics_df, on=grouping_cols, how='inner')
                results[metric_name] = {
                    'merged_df': merged_df,
    ...
    def process_per_frame_data(self, pck_df: pd.DataFrame, metrics_config: Dict[str, str]) -> pd.DataFrame:
        """Process video data for per-frame analysis."""
        combined_rows = []
        grouping_cols = self.config.get_grouping_columns()
        
        if not grouping_cols:
            print("Warning: No grouping columns found. Cannot perform per-frame analysis.")
            return pd.DataFrame()
        ...
        for group_name, group_data in grouped_pck_df:
            ...
            if video_metrics:
                combined_rows.extend(
                    self._combine_video_metrics_with_pck(group_data, video_metrics, video_row_data)
                )
    ...
    def _find_humaneva_video(self, video_row: pd.Series) -> str:
        ...
        video_path = os.path.join(self.config.video_directory, subject, action, video_filename)
        ...
        if hasattr(self.config, 'sync_data') and self.config.sync_data:
            try:
                ...
                if self.config.camera_column:
                    ...
                    synced_start_frame = self.config.sync_data.data[subject_key][action_key][camera_index]
