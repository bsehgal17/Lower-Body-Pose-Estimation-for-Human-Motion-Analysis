        ...
        for i, (video_name, video_data) in enumerate(analysis_results.items()):
            if i < 2:  # Show first 2 videos
                ...
                for key, value in video_data.items():
                    if key == "pck_scores" and isinstance(value, dict):
                        pck_len = len(value)
                        print(f"      {key}: {pck_len} PCK metrics loaded")
                    elif key == "avg_brightness" and isinstance(value, dict):
                        brightness_len = len(value)
                        print(
                            f"      {key}: {brightness_len} joints with brightness data"
                        )
                    elif isinstance(value, dict) and key == "brightness_summary":
                        print(f"      {key}: <detailed brightness data>")
