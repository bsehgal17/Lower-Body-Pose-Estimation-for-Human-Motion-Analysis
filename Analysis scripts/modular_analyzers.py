...
class ANOVAAnalyzer(BaseAnalyzer):
    ...
    def _clean_data(self, df: pd.DataFrame, metric_name: str) -> pd.DataFrame:
        ...
        if cleaned_df.isnull().values.any():
            ...
            cleaned_df.loc[:, columns_to_fill] = cleaned_df.loc[:, columns_to_fill].fillna(0)
    ...
    def _create_bins(self, df: pd.DataFrame, metric_name: str) -> pd.DataFrame:
        ...
        df[f'{metric_name}_bin'] = pd.cut(df[metric_name], bins=bins, labels=labels, right=False)
        return df
    
    def _run_anova_tests(self, df: pd.DataFrame, metric_name: str) -> List[Dict[str, Any]]:
    ...
    def _save_results(self, results: List[Dict[str, Any]], metric_name: str):
        ...
        save_path = os.path.join(self.config.save_folder, f'{metric_name}_anova_results.xlsx')
...
class BinAnalyzer(BaseAnalyzer):
    ...
    def _compute_pck_statistics(self, df: pd.DataFrame, metric_name: str) -> pd.DataFrame:
        """Compute PCK statistics per bin."""
        bin_column = f'{metric_name}_bin'
        pck_summary_list = []
        
        for pck_col in self.config.pck_per_frame_score_columns:
            stats = df.groupby(bin_column)[pck_col].agg(
                mean_pck='mean',
                median_pck='median',
                std_pck='std',
                count='count'
...
class PCKFrameCountAnalyzer(BaseAnalyzer):
    ...
    def _save_results(self, results_df: pd.DataFrame):
        ...
        summary_file = os.path.join(self.config.save_folder, "pck_score_frame_counts.xlsx")
...
class AnalyzerFactory:
    ...
    def create_analyzer(cls, analyzer_type: str, config) -> BaseAnalyzer:
        ...
        if analyzer_type not in cls._analyzers:
            raise ValueError(f"Unknown analyzer type: {analyzer_type}. Available: {list(cls._analyzers.keys())}")
