...
input_folder = (
    r"/storage/Projects/Gaitly/bsehgal/lower_body_pose_est/HumanEva_walking"
)
output_folder = r"/storage/Projects/Gaitly/bsehgal/lower_body_pose_est/real_noise"
...
def add_realistic_noise(image):
    ...
    noisy = np.random.poisson(image.astype(
        np.float32)).clip(0, 255).astype(np.uint8)
...
def process_video(
    input_path, output_path, brightness_factor=40, target_res=(1280, 720)
):
