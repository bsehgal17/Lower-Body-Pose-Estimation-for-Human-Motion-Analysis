...
def add_realistic_noise(
    ...
    noisy_poisson = (
        np.random.poisson(image.astype(np.float32) * poisson_scale) / poisson_scale
    )
    noisy_poisson = np.clip(noisy_poisson, 0, 255).astype(np.uint8)
    gaussian_noise = np.random.normal(0, gaussian_std, image.shape).astype(np.int16)
...
def apply_motion_blur(image: np.ndarray, kernel_size: int = 5) -> np.ndarray:
    if kernel_size % 2 == 0:
        kernel_size += 1
        logger.warning(f"Motion blur kernel size adjusted to odd: {kernel_size}")
