...
class FilterLibrary:
    @staticmethod
    def butterworth(data, cutoff=5, fs=60.0, order=10):
        if len(data) <= order:
    ...
    @staticmethod
    def wiener(data, window_size=3):
        if len(data) < window_size:
            return np.array(data)
        return wiener(data, mysize=window_size)

    @staticmethod
    def kalman(data, process_noise=1.0, measurement_noise=1.0):
        if len(data) == 0:
            return np.array([])
    ...
    @staticmethod
    def kalman_rts(data, process_noise=1.0, measurement_noise=1.0):
        if len(data) == 0:
            return np.array([])
        kf = FilterLibrary._init_kalman(process_noise, measurement_noise)
