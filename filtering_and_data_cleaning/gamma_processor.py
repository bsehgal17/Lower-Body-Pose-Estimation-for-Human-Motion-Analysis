"""
Gamma Correction Pipeline Processor

This module provides a pipeline processor for applying gamma correction to video datasets,
...
from ..config.enhancement_config import GammaConfig
from .gamma_enhancement import (
    GammaEnhancer,
...
class GammaProcessor:
    """
    A processor class for applying gamma correction to video datasets based on YAML configuration.
    """

    def __init__(self, config_path: Union[str, Path]):
        """
        Initialize the gamma processor with a configuration file.
        ...
        self.gamma_config = self._extract_gamma_config()
        self.enhancer = GammaEnhancer(
            gamma=self.gamma_config.gamma,
        )

        logger.info(f"Gamma processor initialized with config: {config_path}")
    ...
    def _extract_gamma_config(self) -> GammaConfig:
        """
        Extracts and validates the gamma configuration from the loaded config.
        ...
        if "gamma" not in self.config:
            raise ValueError(
                "Configuration error: 'gamma' section is missing from the YAML file."
            )

        gamma_section = self.config["gamma"]

        # Use global paths as a fallback for input/output directories
        if "input_dir" not in gamma_section:
            gamma_section["input_dir"] = self.config.get("paths", {}).get("input_dir")
        if "output_dir" not in gamma_section:
            gamma_section["output_dir"] = self.config.get("paths", {}).get("output_dir")

        try:
            # Directly create the config object; this will fail if keys are missing
            gamma_config = GammaConfig(**gamma_section)

            # Ensure file_extensions is a tuple
            gamma_config.file_extensions = tuple(gamma_config.file_extensions)

        except TypeError as e:
            # Catches errors from missing arguments in the dataclass constructor
            raise ValueError(
                f"Missing required parameter in 'gamma' section of the config file. Details: {e}"
            )

        logger.info(f"Gamma config extracted: {gamma_config}")
        return gamma_config
    ...
    def process_single_video(
        ...
        """
        Process a single video file with gamma correction.
        ...
        if output_path is None:
            if self.gamma_config.output_dir:
                output_dir = Path(self.gamma_config.output_dir)
                output_dir.mkdir(parents=True, exist_ok=True)
                output_path = output_dir / f"gamma_{input_path.name}"
            else:
                output_path = input_path.parent / f"gamma_{input_path.name}"
        else:
        ...
        success = self.enhancer.enhance_video(
            ...
            color_space=self.gamma_config.color_space,
    ...
    def process_batch(self) -> bool:
        ...
        try:
            batch_enhance_videos(
                ...
                gamma=self.gamma_config.gamma,
                color_space=self.gamma_config.color_space,
                file_extensions=self.gamma_config.file_extensions,
            )
    ...
    def process_dataset(self, dataset_config: Optional[Dict[str, Any]] = None) -> bool:
        """
        Process videos according to dataset configuration.
        ...
        Returns:
            bool: True if successful, False otherwise
        """
        if dataset_config and "sync_data" in dataset_config:
            # Process structured dataset with sync data
            return self._process_structured_dataset(dataset_config["sync_data"])
        else:
            # Process as simple batch
            return self.process_batch()

    def _process_structured_dataset(self, sync_data: Dict[str, Any]) -> bool:
        """
        Process videos organized in a structured dataset format (subject/action/video).

        Args:
            sync_data (Dict[str, Any]): Synchronization data containing subject/action structure

        Returns:
            bool: True if all videos processed successfully, False otherwise
        """
        base_input_dir = Path(self.gamma_config.input_dir)
        base_output_dir = Path(self.gamma_config.output_dir)
        ...
        for subject, actions in data.items():
            ...
            for action, sync_values in actions.items():
                ...
                video_files = []
                for ext in self.gamma_config.file_extensions:
                    video_files.extend(action_input_dir.glob(f"*{ext}"))

                # Process each video file
                for video_file in video_files:
                    total_count += 1
                    output_file = action_output_dir / f"gamma_{video_file.name}"
    ...
    def get_enhancement_statistics(
        ...
        Returns:
            Dict[str, Any]: Statistics including brightness changes, histogram differences, etc.
        """
        input_path = Path(input_path)
        output_path = Path(output_path)
        ...
        try:
            while True:
                ...
                contrast_change = contrast_output - contrast_input
                ...
                stats["contrast_changes"].append(contrast_change)
        ...
        if stats["frames_processed"] > 0:
            ...
            stats["avg_contrast_change"] = np.mean(stats["contrast_changes"])
            ...
            stats["contrast_change_std"] = np.std(stats["contrast_changes"])
